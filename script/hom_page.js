...
if (!localStorage.getItem('childAccess')) {
    window.location.href = "/lera_game/index.html";
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    menuToggle.addEventListener('click', function () {
        ...
        // Показываем или скрываем оверлей вместе с меню
        if (isActive) {
    ...
    function loadLessonsStructure() {
        fetch('/lera_game/data/lessons.json') // Путь к нашему новому файлу
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                // Используем DocumentFragment для минимизации операций с DOM и ускорения отрисовки
                ...
                accordion.appendChild(fragment); // Добавляем все элементы в DOM за одну операцию
    ...
    function setupSuperGameModal() {
        ...
        startSuperGameBtn.addEventListener('click', () => {
            ...
            const marathonUrl = `/lera_game/templates/super_game_template.html?games=${gameIds}`;
    ...
    function createLessonLink(lesson) {
        ...
        const lessonUrl = lesson.url ? lesson.url : `/lera_game/templates/lesson_template.html?game=${lesson.id}`;
